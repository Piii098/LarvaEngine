...
Tile::Tile(GameObject* parent, int tileId, const Vector2Int& position, const std::string& tileSetName)
    : GameObject(parent)
    , _tileId(tileId)
    , _tileSize(0)
...
void Tile::SetCollider(bool hasCollider) {
    if (hasCollider && !_boxComp) {
        // 衝突判定コンポーネントの生成
		CreateComponent<BoxComponent2D>(true, false);
...
TileMapComponent::TileMapComponent(GameObject& parent, int drawLayer)
    ...
    , _tileMapManager(_parent.GetScene().GetManager().GetGame().GetTileMapManager())
