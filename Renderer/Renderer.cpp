...
bool Renderer::LoadShaders() {
	...
	if (!_spriteShader->Load("Renderer/Shaders/Sprite.vert", "Renderer/Shaders/LightSprite.frag")) {
		SDL_Log("Failed to laod SpriteShader");
		return false;
	}

	_backgroundShader = new Shader();
	if (!_backgroundShader->Load("Renderer/Shaders/Background.vert", "Renderer/Shaders/Background.frag")) {
	...
	if (!_retroShader->Load("Renderer/Shaders/FrameBuffer.vert", "Renderer/Shaders/RetroEffect.frag")) {
		SDL_Log("Failed to laod BlurShader");
		return false;
	}

	_upscaleShader = new Shader();
	if (!_upscaleShader->Load("Renderer/Shaders/FrameBuffer.vert", "Renderer/Shaders/Upscale.frag")) {
		SDL_Log("Failed to laod BlurShader");
		return false;
	}
	
	_bloomFinalShader = new Shader();
	if (!_bloomFinalShader->Load("Renderer/Shaders/FrameBuffer.vert", "Renderer/Shaders/BloomFinal.frag")) {
