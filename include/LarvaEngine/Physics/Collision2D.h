...
struct LineSegment2D{
	...
	Vector2Int _start;  ///< 線分の始点
	Vector2Int _end;    ///< 線分の終点
};
...
struct AABB2D{
	...
	Vector2Int _min;	///< AABBの最小値(左下)
	Vector2Int _max;	///< AABBの最大値(右上)
};
...
struct ConvexPolygon2D{
	...
	std::vector<Vector2Int> _vertices;	///< 頂点の配列
};
...
bool TestSidePlane(float start, float end, float negd, const Vector2Int& norm, std::vector<std::pair<float, Vector2Int>>& out); ///< 2D交差判定の補助関数
bool Intersect(const LineSegment2D& l, const AABB2D& b, float& outT, Vector2Int& outNorm); ///< 線分とAABBの交差判定
